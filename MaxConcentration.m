...
for idx = 1:length(current_values)
    ...
    max_WESC_CLi = 0;  % Initialize the single-point maximum value
    best_t_max = 0;
    ...
    for t = 1:1:t_max
        ...
        % Calculate WESC + CLi for each current point
        current_WESC_CLi = WESC + CLi;
        [max_current_point, idx_point] = max(current_WESC_CLi);

        % Check if it is the single-point maximum value
        if max_current_point > max_WESC_CLi
            max_WESC_CLi = max_current_point;
            best_t_max = t;
        end
    end

    % Save the results (current, t_max, corresponding maximum single-point value)
    results(idx, :) = [I, best_t_max, max_WESC_CLi];
    fprintf('I = %d, t_max = %d, max(WESC + CLi) = %.2f\n', I, best_t_max, max_WESC_CLi);
end

% 输出最终结果
disp('results:');
disp(array2table(results, 'VariableNames', {'Current', 't_max', 'Max_WESC_CLi'}));
